...
bootstrap_se = function(year, quant, n = 50000, num = 40) {
  ...
  x = replicate(num, sample(data[, tox], n, replace = TRUE, prob = data$hispanic/sum(data$hispanic)), simplify = "array")
  m = apply(x, 2, function(x) quantile(x, quant))
  hisp_sd = sd(m)
  ...
  m = apply(x, 2, function(x) quantile(x, quant))
  other_sd = sd(m)
  return(c(year, white_sd, black_sd, hisp_sd, other_sd))
}
...
get_bootstrap = function(quant) {
  ...
  names(errors) = c("year", "white_sd", "black_sd", "hispanic_sd", "other_sd")
  
  write.csv(errors, paste0("data/temp_bootstrap_race_se_", (quant*100), ".csv"), row.names = FALSE)
  print(head(errors))
}
