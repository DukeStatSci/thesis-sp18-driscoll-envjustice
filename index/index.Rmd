...
RSEI data is extremely fine-grained, however it contains many inconsistencies over time, and needs to be aggregated to the census geography. By aggregating RSEI data, we are able to calculate toxicity measures for each block group. In combination with the census data, this allows us to estimate distributions of toxicity experienced by minority groups in the United States. Additionally, we are able to analyze macro level trends over time, such as the overall geography of toxicity in the United States, how the 5^th^, 50^th^, and 95^th^ percentiles of toxicity experienced by each race group varies over time, and how protective higher incomes are for each race group. 
...
The results are incorporated into a publicly available app that allows users to investigate toxicity in their area relative to the U.S. at large, and investigate the differences in local racial toxicty burden.
