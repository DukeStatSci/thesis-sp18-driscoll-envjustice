...
```{r}
sim_tox = function(cur_data, perc, legend = FALSE, smooth = FALSE) {
  g = ggplot(data = cur_data) + 
        labs(x = "Year", y = "Toxicity", title = paste0(perc, "th PCTL Group Toxicity"))
  if (legend == FALSE) {
    g = g + guides(color = FALSE)
  ...
  if (smooth == TRUE) {
    g = g +
     ...
     g = g +
      geom_line(aes(x = year, y = log(white), color = "white")) +
      geom_line(aes(x = year, y = log(black), color = "black")) + 
      geom_line(aes(x = year, y = log(hispanic), color = "hispanic")) + 
      geom_line(aes(x = year, y = log(other), color = "other"))
  }
  g = g + geom_line(aes(x = year, y = log(sim_b), color = "black"), linetype = "dashed", size = 0.7) + 
    geom_line(aes(x = year, y = log(sim_o), color = "other"), linetype = "dashed", size = 0.7) 
  g
}

grid.arrange(sim_tox(perc05, 5), legend, nrow = 2,heights = c(10, 1))
grid.arrange(sim_tox(perc50, 50), legend, nrow = 2,heights = c(10, 1))
grid.arrange(sim_tox(perc95, 95), legend, nrow = 2,heights = c(10, 1))
```
