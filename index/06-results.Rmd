...
To illustrate the form this model could take, we build preliminary quantile regression models [@quantreg] on the 5^th^, 50^th^, and 95^th^ percentiles of the toxicity distribution. To predict log toxicity, we use the year, demographic information, population density and quadratic terms. This model operates on a tract level, using percentage of each population group, population density, and would ideally also include variables like unemployment rates, percentage renters, etc.
